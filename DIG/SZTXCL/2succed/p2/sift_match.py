...
def convolve(kernel, img, padding, strides):
    ...
    if len(mat_size) == 3:
        ...
        for i in range(mat_size[-1]):
            pad_mat = np.pad(img[:, :, i], ((padding[0], padding[1]), (padding[2], padding[3])), 'constant')
            temp = []
            for j in range(0,mat_size[0],strides[1]):
                temp.append([])
                for k in range(0,mat_size[1],strides[0]):
                    val = (kernel * pad_mat[j:j + filter_size[0], k:k + filter_size[1]]).sum()
                    temp[-1].append(val)
        ...
        channel = []
        pad_mat = np.pad(img, ((padding[0], padding[1]), (padding[2], padding[3])), 'constant')
        for j in range(0, mat_size[0], strides[1]):
            channel.append([])
            for k in range(0, mat_size[1], strides[0]):
                val = (kernel * pad_mat[j:j + filter_size[0], k:k + filter_size[1]]).sum()
...
#调整极值点位置
def adjustLocalExtrema(DoG,o,s,x,y,contrastThreshold,edgeThreshold,sigma,n,SIFT_FIXPT_SCALE,SIFT_MAX_INTERP_STEPS = 5,SIFT_IMG_BORDER = 5):
    '''
...
def LocateKeyPoint(DoG,sigma,GuassianPyramid,n,BinNum = 36,contrastThreshold = 0.04,edgeThreshold = 10.0,
                   SIFT_ORI_SIG_FCTR = 1.52, SIFT_ORI_PEAK_RATIO = 0.8,SIFT_FIXPT_SCALE = 1):
...
def SIFT(img,showDoGimgs = False,SIFT_SIGMA = 1.6,SIFT_INIT_SIGMA = 0.5,n = 2):
    '''
    :param img: 图像
    :param showDoGimgs:
    :param SIFT_SIGMA:
