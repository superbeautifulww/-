...
def Cropping(img):
    ...
    # 从四个角开始遍历图像
    top_left_corner = (0, 0)
    top_right_corner = (gray_img.shape[1] - 1, 0)
    bottom_left_corner = (0, gray_img.shape[0] - 1)
    bottom_right_corner = (gray_img.shape[1] - 1, gray_img.shape[0] - 1)

    # 从四个角开始逐渐向内部遍历图像，找到第一个不是黑色的像素点
    top_left = find_black_edge(gray_img, top_left_corner, (1, 1))
    top_right = find_black_edge(gray_img, top_right_corner, (-1, 1))
    bottom_left = find_black_edge(gray_img, bottom_left_corner, (1, -1))
    bottom_right = find_black_edge(gray_img, bottom_right_corner, (-1, -1))
...
def find_black_edge(image, start_point, direction):
    x, y = start_point
    dx, dy = direction
...
def get_homo(originimg1,originimg2,img1,img2,num_iterations):
    k1, d1 = S.SIFT(img1)
    k2, d2 = S.SIFT(img2)
    ...
    '''
    print("k1_points:",k1_points.shape)
    print("k2_points:",k2_points.shape)
    H=R.find_homography_ransac(k1_points,k2_points,num_iterations)
    print(H)
    #H2, _ = cv2.findHomography(k1_points, k2_points, cv2.RANSAC, 5.0)
...
def stitch_image(img1, img2, H):
    ...
    # img1_transform =perspective_transform(img1_dims,H)
    ...
    result_img = Cropping(result_img)
    plt.imsave('result_img.jpg', result_img)
