                      ...
                      QVector<QVector<double>> prob_field,QImage img)
{
    ...
    for (int y = 0; y < img.height(); y++) {
        for (int x = 0; x < img.width(); x++) {
            ...
            if (maxp!=0&&maxp==p_road)
            {
                resimg.setPixel(x, y, qRgb(255, 255, 0));
            }
            else if(maxp!=0&&maxp==p_water)
            {
                resimg.setPixel(x, y, qRgb(0,0, 255));
            }
            else if(maxp!=0&&maxp==p_building)
            {
                resimg.setPixel(x, y, qRgb(255, 0, 0));
            }
            else{
                resimg.setPixel(x, y, qRgb(0, 128, 0));
            }
            /*
            else if(maxp!=0&&maxp==p_field)
            {
                resimg.setPixel(x, y, qRgb(0, 128, 0));
            }
            else{
